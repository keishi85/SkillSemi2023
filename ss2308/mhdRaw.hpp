...
public:
    ...
    std::map<std::string, std::string>loadText(std::string path);
...
std::map<std::string, std::string>LoadMhdRaw::loadText(std::string path){
    std::ifstream inFile(path);
...
void LoadMhdRaw::createMhdFile(const std::string savePath){
    ...
    std::string dataType;
    if (this->dic["WindowProcessing"] != "True") {
        dataType = "MET_SHORT";
    } else {
        dataType = "MET_UCHAR";
    }
