...
def train(data_dir, output_path, model_name='', class_weight=False,
    ...
    train_val_index, test_index = train_test_split(np.arange(len(dataset)),
                                                   test_size=0.2,
                                                   random_state=seed,
                                                   stratify=case_numbers)
    ...
    for _fold, (train_index, val_index) in enumerate(gkf.split(train_val_index, groups=case_numbers[train_val_index])):
        ...
        # Find the class with the maximum number of samples (which is "positive" class in your case)
        max_class_count = max(class_counts.values())

        # Create a list to store indices of samples to be duplicated
        indices_to_duplicate = []

        # Determine how many times to duplicate to match the number of samples in the "positive" class
        num_duplicates = max_class_count - min_class_count

        count = 0
        while True:
            # Only perform oversampling if the "negative" class has fewer samples than the "positive" class
            if num_duplicates > 0:
                # Duplicate samples from the "negative" class to match the number of samples in the "positive" class
                for idx in train_index:
                    _, target = dataset.samples[idx]
