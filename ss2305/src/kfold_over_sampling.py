...
def train(data_dir, output_path, model_name='', class_weight=False,
    ...
    # Get case number
    case_numbers = np.array([extract_case_number(path) for path, _ in dataset.imgs])
    # for n in case_numbers:
    #     print(n)
    print(f'len(case_numbers) : {len(case_numbers)}')
    ...
    for _fold, (train_index, val_index) in enumerate(gkf.split(np.arange(len(dataset)),
                                                               groups=case_numbers)):
        _fold += 1
        ...
        old_train_dataset = torch.utils.data.dataset.Subset(dataset, train_index)   # To class weights
        ...
        for idx in train_index:
            _, target = dataset.samples[idx]
            class_counts[target] += 1
        print(f'class_counts : {class_counts}')

        # Find the class with the minimum number of samples
        min_class_count = min(class_counts.values())
        # Find the class with the maximum number of samples (which is "negative" class in your case)
        max_class_count = max(class_counts.values())

        # Create a list to store indices of samples to be duplicated
        indices_to_duplicate = []

        # Determine how many times to duplicate to match the number of samples in the "negative" class
        num_duplicates = max_class_count - min_class_count

        count = 0
        while True:
            # Only perform oversampling if the "positive" class has fewer samples than the "negative" class
            if num_duplicates > 0:
                # Duplicate samples from the "positive" class to match the number of samples in the "negative" class
                for idx in train_index:
                    _, target = dataset.samples[idx]
                    if target == dataset.class_to_idx['positive']:
                        indices_to_duplicate.append(idx)
                num_duplicates -= min_class_count
                count += 1
            else:
                break
        ...
        rn.seed(seed)
        rn.shuffle(indices_to_duplicate)

        # Duplicate the indices
        duplicated_indices = indices_to_duplicate
        print(duplicated_indices)

        # Create a new list to store indices of duplicated samples
        new_indices = list(train_index) + duplicated_indices

        # Update the training index with the new indices
        train_index = np.array(new_indices)
        ...
        train_labels = [label for _, label in [dataset.samples[i] for i in train_dataset.indices]]
        train_class_counts = Counter(train_labels)
        print(f'train_class_counts : {train_class_counts}')

        # 検証データセットのクラスごとのデータ数をカウント
        val_labels = [label for _, label in [dataset.samples[i] for i in val_dataset.indices]]
        ...
        # Displayed if image data is 18 or less or 23 or more for each case
        path_list = []
        for idx in train_dataset.indices:
            path, label = dataset.imgs[idx]
            path_list.append(path)
            # print(f"Path: {path}, Label: {label}")
        count_path(path_list)
        ...
        if class_weight == True:
            class_weights = class_weighting.get_class_weights(dataset=old_train_dataset)
...
if __name__ == "__main__":
    ...
    ave_val_acc = train(data_dir, model_name='ResNet', output_path=output_path,
                        learning_rate=lr,
                        class_weight=False,
                        batch_size=batch_size,
                        max_epoch=max_epoch)
