...
def train(data_dir, output_path, model_name='', class_weight=False,
    ...
    # MNISTデータセットの読み込み
    train_dataset = datasets.MNIST(root=data_dir, train=True, download=True, transform=data_transforms['train'])
    val_dataset = datasets.MNIST(root=data_dir, train=False, download=True, transform=data_transforms['val'])
    ...
    for idx, (path, target) in enumerate(train_dataset):
    ...
    #         for idx, (path, target) in enumerate(old_train_dataset):
    ...
    # train_dataset = torch.utils.data.dataset.Subset(old_train_dataset, train_index)
