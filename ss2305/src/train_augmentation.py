...
def train(data_dir, output_path, model_name='', class_weight=False,
    ...
    # Report log
    sys.stdout = OutputLogger(f'{output_path}/memo.txt')
    ...
    all_indices = np.arange(len(datasets.ImageFolder(root=train_dir, transform=data_transforms['train'])))
    train_dataset = CustomDataset(root=train_dir, transform=data_transforms['train'], indices=all_indices)
    ...
    # CustomDatasetの全サンプルをイテレートし、クラスごとにカウント
