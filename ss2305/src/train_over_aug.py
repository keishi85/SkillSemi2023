...
def train(data_dir, output_path, model_name='', class_weight=False,
    ...
    old_train_dataset = datasets.ImageFolder(root=train_dir, transform=data_transforms['train'])
    ...
    for idx, (path, target) in enumerate(train_dataset.samples):
    ...
    while True:
        ...
        if num_duplicates > 0:
            ...
            for idx, (path, target) in enumerate(train_dataset.samples):
                _, target = old_train_dataset.samples[idx]
                if target == old_train_dataset.class_to_idx['positive']:
    ...
    new_indices = list(range(len(old_train_dataset))) + indices_to_duplicate

    # Update the training index with the new indices
    train_index = np.array(new_indices)
    train_dataset = torch.utils.data.dataset.Subset(train_dataset, train_index)

    # トレーニングデータセットのクラスごとのデータ数をカウント
    train_labels = [train_dataset.dataset.samples[i][1] for i in np.arange(len(train_dataset))]
    ...
    for idx in train_dataset.indices:
        path, label = train_dataset.dataset.imgs[train_dataset.indices[idx]]
